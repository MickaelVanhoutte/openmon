...
function exportMoves() {
	...
	pokemonIds
		...
		.forEach((id) => {
			...
			let pokemonMoves = movesAssociation
				...
				.filter(
					(move) =>
						Number.parseInt(move.pokemon_move_method_id) === 1 ||
						Number.parseInt(move.pokemon_move_method_id) === 2 ||
						Number.parseInt(move.pokemon_move_method_id) === 10
				);
			...
			pokemonMoves.forEach((move) => {
				...
				let moveEffectFound = moveEffectsFromJson.find(
					(m) => m.move_effect_id === moveFound?.effect_id
				);
				...
				if (moveFound && typeById[moveFound.type_id] && moveEffectFound) {
					movesAssociationArray.push({
						...
						move: {
							...
							category:
								moveFound.damage_class_id === 1
									? 'no-damage'
									: moveFound.damage_class_id === 2
										? 'physical'
										: 'special',
	...
	movesAssociationArray = movesAssociationArray.filter((move, index, self) => {
		return (
			index ===
			self.findIndex((t) => t.pokemon_id === move.pokemon_id && t.move.id === move.move.id)
		);
	});
	console.log(movesAssociationArray.length);
	fs.writeFile(
		'/Users/perso/workspace/perso/openmon/src/assets/data/final/beta/move-associations.json',
		JSON.stringify(movesAssociationArray),
		(error) => {
			// throwing the error
