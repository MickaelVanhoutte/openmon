...
import { Objective, ObjectiveState, QUESTS, Quest, QuestState, Flags } from "../../scripting/quests";
...
export class QuestManager {
	...
	validateObjective(questId: number, objectiveId: number): void {
		...
		if (objective) {
			...
			if (questState) {
				...
				this.questStates.push(new QuestState(
					questId,
					false,
	...
	isOutsideQuestBounds(futurePosition: Position, area: { start: Position; end: Position; }): boolean {
		return futurePosition.x < area.start.x ||
