...
finalDex.forEach((pokemon) => {
	...
	if (originalPokemon && patchedPokemon) {
		...
		if (OgTotalStats !== finalTotalStats || typesIfDifferent || newMoves.length > 0) {
			changes.push({
				id: pokemon.id,
...
function getNewMoves(originalPokemon, patchedPokemon) {
	...
	let newMoves = patchedMoves.filter(
		(move) => Number.parseInt(move.pokemon_id) === patchedPokemon.id
	);
	let ogMoves = originalMoves.filter(
		(move) => Number.parseInt(move.pokemon_id) === patchedPokemon.id
	);
	...
	result = newMoves
		.filter(
			(move) =>
				!ogMoves.find(
					(ogMove) =>
						move.move_id === ogMove.move_id &&
						move.pokemon_move_method_id === ogMove.pokemon_move_method_id
				)
		)
		.map((m) => {
			return {
...
changes.forEach((change) => {
	...
	if (change.totalStats !== change.originalTotalStats) {
		console.log(
			'\t' +
				'Base stats changed: +' +
				(change.totalStats - change.originalTotalStats) +
				'( ' +
				change.originalTotalStats +
				' -> ' +
				change.totalStats +
				' )'
		);
