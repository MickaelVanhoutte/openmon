<div class="__wrapper"
     use:dndzone={{ items: list, dropFromOthersDisabled: finalList ? list.length === 4 : false, flipDurationMs: 200, dragDisabled: finalList ? list.length === 1 : false}}
...
<script lang="ts">
    ...
    function handleDndFinalize(e) {
        list = finalList ? e.detail.items.map(item => {
            return new MoveInstance(item.id, item.name, item.type, item.category, item.power,  item.accuracy,item.pp, item.priority, item.effect, item.effectChance, item.description, item.level);
        }) : e.detail.items;
    }
